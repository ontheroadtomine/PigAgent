...
本文档用于梳理 Codex、Claude/Claude Code 在上下文管理上的设计思路，并基于 Nexa 当前实现提出一套可落地的 context 管理方案。
...
- 项目级规则可以放入 `NEXA.md` 或 `.nexa/memory.md`。
...
## 4. Nexa 当前 Context 现状

### 4.1 当前实现

Nexa 当前 `AgentLoop.run()` 每次会新建 messages：
